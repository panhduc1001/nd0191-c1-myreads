...
function App() {
	...
	const searchBooks = async (query) => {
		if (query) {
			const results = await search(query); // search for books with the given query, return up to 10 results
			console.log(results);
			if (results.error) {
				setBooks([]);
			} else {
	...
	const addBook = (book, newShelf) => {
		book.shelf = newShelf;
		setBooks([...books, book]);
	};

	const removeBook = (removedBook) => {
		setBooks(books.filter((book) => book.id !== removedBook.id));
	};
	...
	const changeShelf = (book, newShelf) => {
		...
		// Update the book's shelf in the backend
